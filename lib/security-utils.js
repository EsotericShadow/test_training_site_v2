...
export const validateInput = {
  ...
  teamMember: (data) => {
    ...
    const sanitizedRole = sanitizeInput.text(data.role);
    if (sanitizedRole.length < 2) {
      return { success: false, error: 'Role must be at least 2 characters' };
    }
    if (sanitizedRole.length > 100) {
      return { success: false, error: 'Role must be less than 100 characters' };
    }
    data.role = sanitizedRole;

    // Validate bio (optional)
    if (data.bio && typeof data.bio === 'string') {
      const sanitizedBio = sanitizeInput.message(data.bio);
      if (sanitizedBio.length > 5000) {
        return { success: false, error: 'Bio must be less than 5000 characters' };
      }
    ...
    // Validate photo_url (optional)
    if (data.photo_url && typeof data.photo_url === 'string') {
      const sanitizedPhotoUrl = sanitizeInput.text(data.photo_url);
      if (sanitizedPhotoUrl.length > 1000) {
        return { success: false, error: 'Photo URL must be less than 1000 characters' };
      }
      if (!validator.isURL(sanitizedPhotoUrl, { require_protocol: true })) {
        return { success: false, error: 'Invalid photo URL' };
      }
      data.photo_url = sanitizedPhotoUrl;
    } else {
      data.photo_url = '';
    }
    ...
    if (data.experience_years !== undefined && data.experience_years !== null) {
      const parsedYears = parseInt(data.experience_years);
      if (isNaN(parsedYears) || parsedYears < 0 || parsedYears > 100) {
        return { success: false, error: 'Experience years must be a number between 0 and 100' };
      }
    ...
    // Validate specializations (optional, must be array of strings)
    if (data.specializations) {
      if (!Array.isArray(data.specializations)) {
        return { success: false, error: 'Specializations must be an array' };
      }
      const sanitizedSpecializations = data.specializations.map(spec => 
        typeof spec === 'string' ? sanitizeInput.text(spec) : ''
      ).filter(spec => spec.length > 0);
      if (sanitizedSpecializations.some(spec => spec.length > 100)) {
        return { success: false, error: 'Each specialization must be less than 100 characters' };
      }
      data.specializations = sanitizedSpecializations;
    } else {
    ...
    if (data.display_order !== undefined && data.display_order !== null) {
      const parsedOrder = parseInt(data.display_order);
      if (isNaN(parsedOrder) || parsedOrder < 0) {
        return { success: false, error: 'Display order must be a non-negative number' };
      }
