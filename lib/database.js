...
const adminSessionsOps = {
  ...
  // MISSING FUNCTION 1: deleteByUserId (called by login route)
  deleteByUserId: async (userId) => {
    try {
      const result = await sql`DELETE FROM admin_sessions WHERE user_id = ${userId}`;
      return { changes: result.rowCount || 0 };
    } catch (error) {
      logger.error('Database error deleting sessions by user ID:', { error: error.message, userId });
      throw new Error('Failed to delete sessions by user ID');
    }
  },

  // MISSING FUNCTION 2: deleteByToken (called by secure-jwt)
  deleteByToken: async (token) => {
    try {
      const result = await sql`DELETE FROM admin_sessions WHERE token = ${token}`;
      return { changes: result.rowCount || 0 };
    } catch (error) {
      logger.error('Database error deleting session by token:', { error: error.message });
      throw new Error('Failed to delete session by token');
    }
  },

  // MISSING FUNCTION 3: getById (called by session-manager)
  getById: async (sessionId) => {
    try {
      ...
      logger.error('Database error getting session by ID:', { error: error.message, sessionId });
      throw new Error('Failed to retrieve session by ID');
    }
  },

  // MISSING FUNCTION 4: deleteById (called by session-manager)
  deleteById: async (sessionId) => {
    try {
      const result = await sql`DELETE FROM admin_sessions WHERE id = ${sessionId}`;
      return { changes: result.rowCount || 0 };
    } catch (error) {
      logger.error('Database error deleting session by ID:', { error: error.message, sessionId });
