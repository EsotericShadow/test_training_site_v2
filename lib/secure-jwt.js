...
async function generateSecureToken(user, request) {
  ...
  // Generate token with expiresIn option only (no manual exp)
  return jwt.sign(enhancedPayload, JWT_SECRET, { 
    algorithm: 'HS256',
    expiresIn: TOKEN_EXPIRY // Using expiresIn option instead of manual exp
  });
}
...
async function verifySecureToken(token, request) {
  try {
    ...
    if (decoded.ipHash && decoded.ipHash !== simpleHash(currentIP)) {
      logger.warn('Token IP mismatch detected', {
        tokenIP: decoded.ipHash,
        currentIP: simpleHash(currentIP),
...
const secureJwt = {
  withAuth: withSecureAuth_export,
  generateToken: generateSecureToken_export,
  verifyToken: verifySecureToken_export,
  authenticate: authenticateSecure_export,
  logout: secureLogout_export,
  utils: {
    getClientIP,
    generateFingerprint,
    checkRateLimit
