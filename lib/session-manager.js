...
import { generateSecureToken, verifySecureToken } from './secure-jwt';
...
async function createSession(user, ipAddress = 'unknown', userAgent = 'unknown') {
  try {
    ...
    return {
      token,
      expiresAt,
      maxAge: 2 * 60 * 60, // 2 hours for secure tokens
    };
  } catch (error) {
  ...
  try {
    ...
    if (!tokenResult.valid) {
      logger.warn('Enhanced session validation failed', { 
        ip, 
        reason: tokenResult.error || 'invalid_token',
        securityLevel: 'enhanced'
      });
      return { valid: false, reason: tokenResult.error || 'invalid_token' };
    }
    ...
    const needsRenewal = tokenResult.needsRefresh || timeLeft < SESSION_CONFIG.renewThreshold;
...
async function terminateSession(sessionId, userId) {
  try {
    ...
    const session = await adminSessionsOps.getById(sessionId);
    
    // Only allow termination if session belongs to user
    if (session && session.user_id === userId) {
      await adminSessionsOps.deleteById(sessionId);
