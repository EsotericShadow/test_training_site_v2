...
async function runSeeding() {
  try {
    ...
    console.log('🚀 Starting content seeding to CMS database...');
...
async function seedCompanyValues() {
  ...
  for (const value of companyValues) {
    await pool.sql`
      ...
      description = EXCLUDED.description, icon = EXCLUDED.icon, display_order = EXCLUDED.display_order
    `;
  }
...
async function seedWhyChooseUs() {
  console.log('🎯 Seeding why choose us points...');
  ...
  console.log('✅ Why choose us points seeded');
}
...
async function seedTeamMembers() {
  ...
  for (const member of teamMembers) {
    await pool.sql`
      ...
      VALUES (${member.name}, ${member.role}, ${member.bio}, ${member.photo_url || null}, ${member.experience_years}, ${member.specializations}, ${member.featured}, ${member.display_order})
  ...
  for (const testimonial of testimonials) {
    await pool.sql`
      ...
      VALUES (${testimonial.client_name}, ${testimonial.client_role}, ${testimonial.company}, 
              ${testimonial.industry || null}, ${testimonial.content}, ${testimonial.rating}, ${testimonial.client_photo_url || null}, ${testimonial.featured})
      ON CONFLICT (client_name) DO UPDATE SET
      client_role = EXCLUDED.client_role, company = EXCLUDED.company, industry = EXCLUDED.industry,
      content = EXCLUDED.content, rating = EXCLUDED.rating, client_photo_url = EXCLUDED.client_photo_url,
...
async function seedHeroStats() {
  ...
  for (const stat of heroStats) {
    await pool.sql`
      ...
      ON CONFLICT (number_text, label) DO UPDATE SET
...
async function seedHeroFeatures() {
  console.log('⭐ Seeding hero features...');
  ...
  for (const course of courses) {
    ...
    await pool.sql`
      ...
      VALUES (${course.slug}, ${course.title}, ${course.description}, ${course.duration}, 
              ${course.audience}, ${categoryId}, ${course.popular}, ${course.image_url || null}, ${course.image_alt})
